...
class DataProvider implements DataProviderInterface {
    ...
    public function getData($page, $sort, $limit, $filter) {
        ...
        foreach($this->columns as $i => $column) {
            if ( $column instanceof ColumnInterface && count($column->getParams()) > 0 ) {
                foreach( $column->getParams() as $j => $param ) {
    ...
    private function addFilter(QueryBuilder $queryBuilder, $filter) {
        ...
        foreach($this->columns as $column) {
            if ( $column instanceof \EMC\TableBundle\Column\TextInterface && $column->isSearchable() ) {
